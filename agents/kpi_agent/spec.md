...
| SINR_LOW | `alerts` CRITICAL/SINR_DEGRADATION | `son_actions` PCI_REOPT_REQUEST | Best-effort `POST /son/pci-reopt` on Controller |
...
| `POLL_INTERVAL_SEC` | `10` (code) / `30` (Docker) | Poll cadence; window fills in `SEQ_LEN × POLL_SEC` |
...
| File | Purpose |
|---|---|
| `kpi_agent.py` | Poll loop, feature extraction, SON action dispatcher |
| `model.py` | `KPIClassifier` — 2-layer BiLSTM + MLP head (PyTorch) |
| `train.py` | Synthetic dataset generation + model training script |
