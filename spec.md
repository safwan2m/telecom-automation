...
**Demand node concept** (Tutschku 1998): traffic per area is a finite set of *demand clusters* D, each with a channel requirement ρ_i, separate from candidate sites S; 10 pre-defined Bangalore clusters, one per area.
...
- `dus[*].cell_ids` is **runtime state** — mutated by `move_cell` / SON load-balancing.
  The shipped `topology.json` is a live snapshot (version 472) whose DU groupings have been
  shuffled by SON and differ from the §5 initial design (12/9/9 design vs 9/6/15 snapshot).
...
All simulators push every `INTERVAL_SEC = 10 s`.
...
(Standard deviations per class are in `train.py:_5G_SPECS / _4G_SPECS`.)
...
| SINR_LOW | Alert CRITICAL(SINR_DEGRADATION); best-effort `POST /son/pci-reopt` on Controller (⚠ no such route exists → 404; see STATUS.md); son_action `PCI_REOPT_REQUEST`. |
...
| `GEMINI_MODEL` | `gemini-2.0-flash` | Gemini model (⚠ in-code default differs from `gemini-2.5-flash` cited in CLAUDE.md; the Docker Compose override sets the deployed model) |
...
> Volatile — tracks progress, open gaps, and acceptance criteria, not design.
> Moved to **[STATUS.md](STATUS.md)** to keep this specification timeless.
