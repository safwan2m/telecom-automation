...
class CellState:
    ...
    def tick(self) -> dict:
        c      = self.cfg
        target = max(0, min(int(c["max_ues"] * load_factor() * random.uniform(0.88, 1.05)), c["max_ues"]))
        ...
        prb_ul = min(95.0, load * 62  * random.uniform(0.88, 1.12))
        return dict(
            connected_ues       = self.ues,
            dl_throughput_mbps  = round(prb_dl * random.uniform(0.85, 1.15), 2),
            ul_throughput_mbps  = round(prb_ul * random.uniform(0.38, 0.55), 2),
            rsrp_dbm            = round(-70 - load * 28 + random.gauss(0, 3), 1),
            sinr_db             = round( 25 - load * 14 + random.gauss(0, 2), 1),
            power_w             = round( 20 + load * 180 + random.gauss(0, 5), 1),
    ...
    def mobility_events(self, neighbours: list["CellState"]) -> list[dict]:
        if self.ues < 2 or not neighbours:
            return []
        n_ho   = max(0, int(self.ues * 0.02 * random.random()))
        events = []
        for _ in range(n_ho):
            if not self.ue_pool:
                break
    ...
    def usage_sample(self, n: int = 8) -> list[dict]:
        ...
        for uid in random.sample(list(self.ue_pool), min(n, self.ues)):
            sl = self.ue_pool[uid]
            if sl == "URLLC":
                dl, ul, lat, jit = random.randint(1_000, 60_000), random.randint(500, 25_000), random.uniform(0.5, 4), random.uniform(0.1, 0.8)
            elif sl == "mMTC":
                dl, ul, lat, jit = random.randint(10, 2_000), random.randint(10, 800), random.uniform(10, 150), random.uniform(2, 30)
            else:
                dl, ul, lat, jit = random.randint(100_000, 5_000_000), random.randint(10_000, 600_000), random.uniform(5, 35), random.uniform(0.5, 6)
            out.append(dict(ue_id=uid, cell_id=self.cell_id, area=self.cfg["area"],
                            ...
                            packet_loss=round(random.uniform(0, 0.004), 4)))
        return out
...
def reconfigure(states: dict[str, CellState], topo: dict) -> tuple[dict[str, CellState], str]:
    """Diff current cell states against topology; add/remove cells; return new cu_id."""
...
def build_points(states: dict[str, CellState], cu_id: str) -> list[Point]:
    ...
    # cell_kpi + mobility + usage
    for state in cell_list:
        m = state.tick()
        c = state.cfg
        points.append(
            Point("cell_kpi")
            .tag("cell_id", state.cell_id).tag("area", c["area"])
            .tag("band", c["band"]).tag("pci", str(c["pci"]))
        ...
        for ev in state.mobility_events(neighbours):
            points.append(
                Point("ue_mobility")
                .tag("ue_id", ev["ue_id"]).tag("source_cell", ev["source_cell"])
                .tag("target_cell", ev["target_cell"]).tag("source_area", ev["source_area"])
                .tag("target_area", ev["target_area"]).tag("event_type", ev["event_type"])
                .tag("du_id", DU_ID)
        ...
        for u in state.usage_sample():
            points.append(
                Point("ue_usage")
                .tag("ue_id", u["ue_id"]).tag("cell_id", u["cell_id"])
                .tag("area", u["area"]).tag("slice_type", u["slice_type"])
                .tag("du_id", DU_ID)
                .field("dl_bytes",    u["dl_bytes"]).field("ul_bytes", u["ul_bytes"])
                .field("latency_ms",  u["latency_ms"]).field("jitter_ms", u["jitter_ms"])
